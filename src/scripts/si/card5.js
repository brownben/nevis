...
export const processData = (data, port) => {
  ...
  if (!helpers.checkUndefined(data.slice(24, 26))) punches.push(processPunch(data, 23, 'S'))
  punches.push(...getPunches(data))
  if (!helpers.checkUndefined(data.slice(26, 28))) punches.push(processPunch(data, 25, 'F'))

  beepStation(port)
  return { siid, punches: fixPunchTimes(punches) }
}

export const getPunches = (data, position = 38, positionInBlock = 0, controls = []) => {
  if (helpers.checkEmptyPunch(data.slice(position, position + 3))) return controls
  else {
    controls.push(processPunch(data, position))

    if (position === 130) return getPunchesExtra(data, 37, controls)
    else if (positionInBlock < 4) return getPunches(data, position + 3, positionInBlock + 1, controls)
...
export const fixPunchTimes = (controls) => {
  ...
  for (const control of controls) {
    if (control.time < lastPunchTime) control.time += helpers.SECONDS_IN_12_HOURS
