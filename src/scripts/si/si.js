...
export default {
  parseData: function (data, port) {
    if (helpers.validityCheck(data)) {
      if (card5.packetAnalyser.inserted(data)) card5.instructionSets.getData(port)
      else if (card10.packetAnalyser.inserted(data)) card10.instructionSets.getData0(port)
      else if (card5.packetAnalyser.data(data)) return card5.processData(data, port)
