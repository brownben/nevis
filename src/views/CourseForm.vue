  ...
  methods: {
    ...
    deleteCourse: function () {
      return this.$database.query('DELETE FROM courses WHERE id=?', this.course.id)
        .then(() => {
          this.$messages.addMessage(`Some Competitors May Not Have a Course Allocated`, 'warning')
    ...
    recalculateCourseResults: function () {
      if (this.course.controls !== this.originalControls) {
        return this.$database.query('SELECT * FROM competitors WHERE event=? AND course=? AND downloaded=true', [this.$route.params.eventId, this.course.id])
          .then(competitors => Promise.all(competitors.map(async competitor => {
            const punches = await this.$database.query('SELECT * FROM punches WHERE competitor=?', [competitor.id])
            return this.recalculateResult(competitor, punches)
          })))
          .then(() => this.$messages.addMessage(`Results for Course "${this.course.name}" Recalculated`))
          .catch(() => this.$messages.addMessage('Problem Recalulating Course Results', 'error'))
      }
    },
