...
test('Calculate Time', () => {
  ...
  expect(wrapper.vm.calculateTime(
    { errors: '' },
    [{ controlCode: 'S', time: 10 }, { controlCode: 'F', time: 19 }]).time)
    .toBe(9)

  expect(wrapper.vm.calculateTime(
    { errors: '' },
    [{ controlCode: 'S', time: 10 }, { controlCode: 'F', time: 130 }]).time)
    .toBe(120)

  expect(wrapper.vm.calculateTime(
    { errors: '' },
    [{ controlCode: 'S', time: 10 }, { controlCode: 'F', time: 139 }]).errors)
    .toBe('')

  expect(wrapper.vm.calculateTime(
    { errors: 'M1' },
    [{ controlCode: 'S', time: 10 }, { controlCode: 'F', time: 19 }]).errors)
    .toBe('M1')

  expect(wrapper.vm.calculateTime(
    { errors: '' },
    [{ controlCode: 'F', time: 19 }]).errors)
    .toBe('MS')
...
test('Recalculate Results', async () => {
  ...
  await wrapper.vm.recalculateResult({ id: 6, name: 'Bob', course: 'world' }, [{ controlCode: 'S' }, { controlCode: 'F' }, { controlCode: '101' }, { controlCode: '102' }, { controlCode: '103' }])
...
test('Recalculate Course Results', async () => {
  const wrapper = shallowMount(CourseForm, {
    stubs: ['router-link'],
    mocks: {
      $database: { connection: {}, connected: true, query: jest.fn().mockResolvedValue([{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }, { id: 5 }]) },
      $route: { params: { eventId: 12 }, path: '' },
      $router: { push: jest.fn() },
      $messages: { addMessage: jest.fn(), clearMessages: jest.fn() },
    },
  ...
  wrapper.setData({ course: { id: 5, name: 'Long', controls: '101,102,103' }, originalControls: '' })
  await wrapper.vm.recalculateCourseResults()
  expect(wrapper.vm.$database.query).toHaveBeenCalledWith('SELECT * FROM competitors WHERE event=? AND course=? AND downloaded=true', [12, 5])
  expect(wrapper.vm.$database.query).toHaveBeenCalledWith('SELECT * FROM punches WHERE competitor=?', [expect.any(Number)])
  expect(wrapper.vm.recalculateResult).toHaveBeenCalledTimes(5)
  expect(wrapper.vm.$messages.addMessage).toHaveBeenLastCalledWith('Results for Course "Long" Recalculated')

  wrapper.vm.recalculateResult = jest.fn().mockRejectedValue()
  await wrapper.vm.recalculateCourseResults()
  expect(wrapper.vm.$messages.addMessage).toHaveBeenLastCalledWith('Problem Recalulating Course Results', 'error')
})
